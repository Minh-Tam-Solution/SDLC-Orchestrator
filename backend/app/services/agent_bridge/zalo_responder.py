...
_COMMAND_REPLIES: dict[str, str] = {
    ...
    "/help": (
        "SDLC Orchestrator — Governance Commands\n\n"
        "[Agent Direct Routing - Sprint 222]\n"
        "  @<ten_agent> <tin nhan> — Goi truc tiep EP-07 agent\n"
        "  VD: @pjm bao cao hien trang du an\n"
        "  VD: @reviewer kiem tra code PR #42\n"
        "  VD: @coder viet unit test module auth\n\n"
        "[Workspace]\n"
        "  /workspace set <project_id> — Gan project vao chat nay\n"
        "  /workspace status — Xem workspace hien tai\n\n"
        "[Governance (gui bang text tu nhien)]\n"
        "  gate status — Danh sach quality gates\n"
        "  gate status <gate_id> — Chi tiet gate\n"
        "  approve gate <gate_id> — Duyet gate\n"
        "  reject gate <gate_id> — Tu choi gate\n"
        "  submit evidence — Nop evidence cho gate\n"
        "  update sprint — Cap nhat sprint hien tai\n"
        "  close sprint — Dong sprint\n"
        "  export audit — Xuat audit log\n"
        "  invite member <email> — Moi thanh vien\n\n"
        "[Multi-Agent Team - Sprints 218-222]\n"
        "  @vote create \"ten\" majority|unanimous — Tao phieu bau\n"
        "  @vote approve \"ly do\" — Bo phieu dong y\n"
        "  @vote reject \"ly do\" — Bo phieu phan doi\n\n"
        "[General]\n"
        "  /help — Hien thi menu nay\n"
        "  /start — Bat dau conversation\n"
        "  /status — Trang thai he thong\n\n"
        "Powered by SDLC Orchestrator v2.0 (EP-07 Multi-Agent)\n"
        "https://sdlc.nhatquangholding.com"
    ),
    "/status": (
        "SDLC Orchestrator Status\n\n"
        "[OK] API: Online\n"
        "[OK] Zalo OA Gateway: Active\n"
        "[OK] EP-07 Multi-Agent: Active\n"
        "Framework: SDLC 6.1.1\n"
        "Version: v2.0 (Sprint 222)\n\n"
        "Gui /help de xem cac lenh governance va @agent routing."
    ),
}
