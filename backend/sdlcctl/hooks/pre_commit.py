...
def run_validation(
    ...
    if result.is_compliant:
        print(f"✓ SDLC 6.0.5 compliant ({result.compliance_score}/100) [{elapsed:.1f}s]")
        return 0
    else:
        print(f"✗ SDLC 6.0.5 validation failed ({result.compliance_score}/100)")
...
def main() -> int:
    ...
    parser = argparse.ArgumentParser(description="SDLC 6.0.5 pre-commit hook")
    parser.add_argument(
        "--path",
        "-p",
        type=Path,
