...
class TestRegistryRoundtrip:
    """Verify registry returns 5 commands with all required fields."""

    def test_get_commands_returns_five(self):
        """get_commands() must return exactly 5 governance commands."""
...
class TestOllamaToolsGeneration:
    ...
    def test_ollama_tools_count(self):
        """to_ollama_tools() must produce 5 tool definitions."""
        from app.services.agent_team.command_registry import to_ollama_tools

        tools = to_ollama_tools()
        assert len(tools) == 5, f"Expected 5 Ollama tools, got {len(tools)}"
...
class TestVietnameseAliases:
    ...
    def test_all_commands_have_vietnamese_alias(self):
        ...
        vietnamese_keywords = ["tạo", "trạng", "nộp", "duyệt", "xuất"]
        for cmd in get_commands():
            has_vn = any(
