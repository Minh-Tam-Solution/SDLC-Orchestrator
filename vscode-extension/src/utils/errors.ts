    ...
    switch (selection) {
        ...
        case 'Install GitHub App':
            await vscode.env.openExternal(
                vscode.Uri.parse('https://github.com/apps/sdlc-orchestrator/installations/new')
            );
            break;
        ...
        case 'Open GitHub Settings':
            await vscode.env.openExternal(
                vscode.Uri.parse('https://github.com/settings/installations')
            );
            break;
        case 'Retry Sync':
 ...
 */
export class GitHubError extends SDLCError {
    constructor(
        code: ErrorCode,
        message: string,
 ...
 */
export function mapGitHubErrorCode(errorCode: string): ErrorCode {
 ...
 */
export function createGitHubError(response: {
    code?: string;
    message?: string;
    retry_after?: number;
    reset_at?: string;
 ...
 */
export function isGitHubErrorCode(code: ErrorCode): boolean {
 ...
 */
export function parseGitHubApiError(error: unknown): SDLCError {
    if (error instanceof SDLCError) {
        return error;
    }
    ...
    if (typeof error === 'object' && error !== null) {
        ...
        if (response) {
            ...
            if (data && typeof data.code === 'string' && data.code.startsWith('github_')) {
                const githubErrorData: Parameters<typeof createGitHubError>[0] = {
