...
suite('CacheService getOrFetch', () => {
    ...
    test('getOrFetch stores fetched data in cache', async () => {
        ...
        const cached = cacheService.get<typeof freshData>('store-test');
        assert.ok(cached);
...
suite('CacheService Project Operations', () => {
    ...
    test('clearProject removes all project-specific cache', async () => {
        ...
        await cacheService.set(CacheKeys.GATES(projectId), [{ id: 'gate1' }]);
    ...
    test('clearProject does not affect other projects', async () => {
        ...
        await cacheService.set(CacheKeys.GATES(project1), [{ id: 'g1' }]);
        await cacheService.set(CacheKeys.GATES(project2), [{ id: 'g2' }]);

        await cacheService.clearProject(project1);

        // Project 1 cleared
        assert.strictEqual(cacheService.get(CacheKeys.GATES(project1)), undefined);

        // Project 2 still exists
        const result = cacheService.get(CacheKeys.GATES(project2));
        assert.ok(result);
    });
});
