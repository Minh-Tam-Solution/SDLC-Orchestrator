...
import { ApiClient, ApiError, Project, Gate, Violation } from '../../services/apiClient';
...
suite('ApiClient Type Definitions', () => {
    ...
    test('Violation severity can be critical, high, medium, or low', () => {
        const severities: Array<Violation['severity']> = [
            'critical',
            'high',
            'medium',
            'low',
...
suite('ApiClient Error Handling', () => {
    ...
    test('ApiError stores response data', () => {
        ...
        const error = new ApiError(422, 'Unprocessable Entity', 'Validation error', responseData);
