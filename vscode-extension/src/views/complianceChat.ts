 ...
 */
export class ComplianceChatParticipant {
    ...
    async handleChatRequest(
        ...
        Logger.info(`Chat request: command=${request.command ?? 'none'}, prompt=${request.prompt}`);
    ...
    private async handleFixCommand(
        ...
        const recommendation = await this.apiClient.getAIRecommendation(
            violationId,
            councilMode
        );
     ...
     */
    private async handleCouncilCommand(
        prompt: string,
        stream: vscode.ChatResponseStream,
        token: vscode.CancellationToken
    ): Promise<vscode.ChatResult> {
