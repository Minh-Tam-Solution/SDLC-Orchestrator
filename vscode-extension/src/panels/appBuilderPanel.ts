 ...
 */
export class AppBuilderPanel {
    public static currentPanel: AppBuilderPanel | undefined;
    private static readonly viewType = 'sdlc.appBuilderPanel';
    ...
    private constructor(
        ...
        private readonly blueprintProvider: BlueprintProvider
    ) {
        this._panel = panel;
        ...
        Logger.info('AppBuilderPanel created');
    }
    ...
    public static createOrShow(
        ...
        AppBuilderPanel.currentPanel = new AppBuilderPanel(
            panel,
            extensionUri,
            codegenApi,
            blueprintProvider
     ...
     */
    public setBlueprint(blueprint: AppBlueprint): void {
     ...
     */
    public updateGeneratedFiles(files: GeneratedFile[]): void {
     ...
     */
    public updateGenerationStatus(status: {
    ...
    public dispose(): void {
        ...
        Logger.info('AppBuilderPanel disposed');
    }
    ...
    private async _handleMessage(message: WebviewMessage): Promise<void> {
        switch (message.command) {
            ...
            case 'updateBlueprint':
                this._updateBlueprint(message.payload as Partial<AppBlueprint>);
                break;
            ...
            case 'removeModule':
                this._removeModule(message.payload as string);
                break;

            case 'addEntity':
                await this._addEntity(message.payload as string);
                break;

            case 'removeEntity':
                this._removeEntity(message.payload as { moduleName: string; entityName: string });
                break;
            ...
            case 'openFile':
                await this._openGeneratedFile(message.payload as string);
                break;
            ...
            case 'loadTemplate':
                await this._loadTemplate(message.payload as string);
                break;
     ...
     */
    private async _loadBlueprint(): Promise<void> {
    ...
    private async _saveBlueprint(): Promise<void> {
        ...
        try {
            ...
            void vscode.window.showErrorMessage(
                `Failed to save blueprint: ${error instanceof Error ? error.message : 'Unknown error'}`
            );
        }
    }
     ...
     */
    private async _createNewBlueprint(): Promise<void> {
     ...
     */
    private _updateBlueprint(updates: Partial<AppBlueprint>): void {
     ...
     * Add a new module
     */
    private async _addModule(): Promise<void> {
        if (this._isLocked) {
     ...
     * Remove a module
     */
    private _removeModule(moduleName: string): void {
        if (this._isLocked) {
            return;
        }
     ...
     */
    private async _addEntity(moduleName: string): Promise<void> {
        if (this._isLocked) {
            void vscode.window.showWarningMessage('Blueprint is locked');
            return;
        }
     ...
     */
    private _removeEntity(data: { moduleName: string; entityName: string }): void {
        if (this._isLocked) {
            return;
        }
     ...
     * Lock the blueprint
     */
    private async _lockBlueprint(): Promise<void> {
     ...
     */
    private async _unlockBlueprint(): Promise<void> {
     ...
     */
    private async _openGeneratedFile(filePath: string): Promise<void> {
     ...
     */
    private async _saveAllFiles(): Promise<void> {
     ...
     */
    private async _loadTemplate(domainId: string): Promise<void> {
        try {
