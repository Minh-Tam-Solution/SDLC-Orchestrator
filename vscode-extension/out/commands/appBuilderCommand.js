...
exports.registerAppBuilderCommand = registerAppBuilderCommand;
 ...
 */
function registerAppBuilderCommand(context, codegenApi) {
    const command = vscode.commands.registerCommand('sdlc.openAppBuilder', async () => {
        await openAppBuilder(context, codegenApi);
    });
    context.subscriptions.push(command);
    logger_1.Logger.info('App Builder command registered');
}
/**
 * Open the App Builder panel
 */
async function openAppBuilder(context, codegenApi) {
    ...
    // Create new panel
    appBuilderPanel = vscode.window.createWebviewPanel('sdlc.appBuilder', '🏗️ SDLC App Builder', vscode.ViewColumn.One, {
        enableScripts: true,
    ...
    appBuilderPanel.webview.onDidReceiveMessage(async (message) => {
        switch (message.command) {
            ...
            case 'loadDomains':
                ...
                break;
            case 'showInfo':
                void vscode.window.showInformationMessage(message.payload);
                break;
        }
 ...
 */
async function loadDomainTemplates(codegenApi, panel) {
    try {
        ...
        logger_1.Logger.warn(`Failed to load domain templates: ${error}`);
 ...
 */
function getAppBuilderHtml(webview, _extensionUri) {
...
//# sourceMappingURL=appBuilderCommand.js.map