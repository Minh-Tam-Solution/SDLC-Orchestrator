...
exports.registerLockStatusCommand = registerLockStatusCommand;
 ...
 */
function registerLockCommand(context, codegenApi) {
    const lockCommand = vscode.commands.registerCommand('sdlc.lock', async (sessionId) => {
        await executeLockCommand(codegenApi, sessionId);
    });
    const unlockCommand = vscode.commands.registerCommand('sdlc.unlock', async (sessionId) => {
        await executeUnlockCommand(codegenApi, sessionId);
    });
    context.subscriptions.push(lockCommand, unlockCommand);
    logger_1.Logger.info('Lock/Unlock commands registered');
}
/**
 * Execute the lock command
 */
async function executeLockCommand(codegenApi, providedSessionId) {
    try {
        ...
        if (status.is_locked) {
            void vscode.window.showWarningMessage(`Contract is already locked since ${formatDate(status.locked_at)} by ${status.locked_by}`);
            return;
        }
        // Ask for lock reason
        ...
        if (result.success) {
            const action = await vscode.window.showInformationMessage(`🔒 Contract locked successfully!\n\nSpec Hash: ${result.spec_hash.substring(0, 16)}...\nVersion: ${result.version}`, 'Copy Hash', 'View Status');
            if (action === 'Copy Hash') {
 ...
 */
async function executeUnlockCommand(codegenApi, providedSessionId) {
    try {
        // Get session ID if not provided
        const sessionId = providedSessionId || await getSessionId(codegenApi);
        if (!sessionId) {
        ...
        // Confirm unlock
        const confirm = await vscode.window.showWarningMessage(`Are you sure you want to unlock this contract?\n\nLocked by: ${status.locked_by}\nLocked at: ${formatDate(status.locked_at)}\nSpec Hash: ${status.spec_hash?.substring(0, 16)}...`, { modal: true }, 'Unlock', 'Cancel');
        if (confirm !== 'Unlock') {
            return;
        }
        // Ask for unlock reason
        const reasonOptions = [
            { label: 'Need to modify specification', reason: 'modification_needed' },
            { label: 'Generation failed, need to retry', reason: 'generation_failed' },
        ];
        ...
        if (result.success) {
            void vscode.window.showInformationMessage(`🔓 Contract unlocked successfully!\n\n${result.message}`);
        }
        else {
 ...
 */
function registerLockStatusCommand(context, codegenApi) {
    // Status command
    const statusCommand = vscode.commands.registerCommand('sdlc.lockStatus', async (sessionId) => {
    ...
    // Verify hash command
    const verifyCommand = vscode.commands.registerCommand('sdlc.verifyHash', async (sessionId) => {
        await executeVerifyHashCommand(codegenApi, sessionId);
    });
    context.subscriptions.push(statusCommand, verifyCommand);
    logger_1.Logger.info('Lock status commands registered');
}
/**
 ...
 */
async function showLockStatus(codegenApi, sessionId) {
    try {
        const status = await codegenApi.getContractStatus(sessionId);
        const statusItems = [
            {
 ...
 */
async function executeVerifyHashCommand(codegenApi, providedSessionId) {
    try {
        // Get session ID if not provided
        const sessionId = providedSessionId || await getSessionId(codegenApi);
        if (!sessionId) {
        ...
        if (result.match) {
            void vscode.window.showInformationMessage('✅ Hash verification successful! The contract specification has not been modified.');
        }
        else {
            void vscode.window.showWarningMessage(`❌ Hash mismatch!\n\nExpected: ${expectedHash.substring(0, 16)}...\nCurrent: ${result.current_hash.substring(0, 16)}...\n\nThe contract specification has been modified.`);
        }
    }
    catch (error) {
        await (0, errors_1.handleError)(error, {
