...
/**
 * SDLC 6.0.0 Stage Definitions (from Framework README.md)
 *
...
[![SDLC 6.0.0](https://img.shields.io/badge/SDLC-6.0.0-blue)](https://sdlc-orchestrator.dev)
...
This project follows **SDLC 6.0.0** framework with **${tier}** tier governance.
...
`,
    ...
    analyzeExistingStructure(workspaceRoot, targetTier) {
        ...
        // SDLC 6.0.0: Only /docs folders are mapped to stages
        // Code folders (src, backend, frontend, tests) are NOT stage-mapped
        // Stage mapping per SDLC 6.0.0 Framework:
            ...
            if (fs.existsSync(path.join(workspaceRoot, expectedFolder))) {
                ...
                for (const pattern of patterns) {
                    if (fs.existsSync(path.join(workspaceRoot, pattern))) {
                        ...
                        recommendations.push(`Consider renaming '${pattern}' to '${expectedFolder}' for SDLC 6.0.0 compliance`);
                        found = true;
                        break;
                    }
                }
        ...
        for (const legacy of legacyFolders) {
            ...
            if (fs.existsSync(legacyPath)) {
                recommendations.push(`Found legacy SDLC 4.9.x folder 'docs/${legacy}'. Run 'sdlcctl migrate' to upgrade to 6.0.0`);
            }
        }
        return {
            existingFolders,
