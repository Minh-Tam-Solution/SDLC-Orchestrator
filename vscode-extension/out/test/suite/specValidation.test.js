...
suite('Spec Validation Test Suite', () => {
     ...
     */
    function validateSpecificationLocal(content, _tier) {
        const errors = [];
        const warnings = [];
        const fieldsFound = [];
        const fieldsMissing = [];
    ...
    function findFieldLine(content, field) {
    ...
    function createResult(valid, errors, warnings, specId, fieldsFound, fieldsMissing) {
        return {
            valid,
            spec_id: specId,
    ...
    // ===========================================================================
    test('Valid SDLC 6.0.0 spec passes validation', () => {
        const validSpec = `---
spec_id: SPEC-0001
...
`;
        ...
        const specIdError = result.errors.find(e => e.code === 'SPC-002' && e.field === 'spec_id');
...
`;
        const result = validateSpecificationLocal(spec);
        const versionError = result.errors.find(e => e.code === 'SPC-002' && e.field === 'version');
...
`;
        const result = validateSpecificationLocal(spec);
        const statusError = result.errors.find(e => e.code === 'SPC-002' && e.field === 'status');
...
`;
        const result = validateSpecificationLocal(spec);
        const tierError = result.errors.find(e => e.code === 'SPC-002' && e.field === 'tier');
...
`;
        const result = validateSpecificationLocal(spec);
        const dateError = result.errors.find(e => e.code === 'SPC-002' && e.field === 'last_updated');
...
`;
        ...
        assert.ok(versionError.line_number && versionError.line_number > 0, 'Should have positive line number');
        // version: is on line 4 of frontmatter, +2 for --- offset = 6
        // The exact number depends on implementation
        assert.ok(versionError.line_number <= 10, `Line number should be reasonable, got ${versionError.line_number}`);
    });
    // ===========================================================================
...
`;
        ...
        assert.ok(result1.errors.some(e => e.code === 'SPC-001'), 'Should use SPC-001 for missing fields');
        // Test SPC-002
        const badFormatSpec = `---
spec_id: BAD
title: "Test"
...
`;
        ...
        assert.ok(result2.errors.some(e => e.code === 'SPC-002'), 'Should use SPC-002 for format errors');
        // Test SPC-004
        const noFrontmatter = '# No frontmatter';
        const result4 = validateSpecificationLocal(noFrontmatter);
        assert.ok(result4.errors.some(e => e.code === 'SPC-004'), 'Should use SPC-004 for missing frontmatter');
    });
});
...
//# sourceMappingURL=specValidation.test.js.map